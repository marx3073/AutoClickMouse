...
namespace AutoClickMouse
{
    public partial class AutoClickMouseLeftButton : Form
    {
        ...
        private class ExecMouseEvent
        {
            public MouseEventFlags UpEvent { get; set; }
            public MouseEventFlags DownEvent { get; set; }
        }

        private ExecMouseEvent SetExecMouseEvent()
        {
            if (RB_Left.Checked)
                return new ExecMouseEvent() { DownEvent = MouseEventFlags.LeftDown, UpEvent = MouseEventFlags.LeftUp };
            return new ExecMouseEvent() { DownEvent = MouseEventFlags.RightDown, UpEvent = MouseEventFlags.RightUp };
        }
        ...
        private void AutoClick(int x, int y)
        {
            ...
            {
                SetCursorPos(x, y);
                var mEvent = SetExecMouseEvent();
                mouse_event((int)(mEvent.DownEvent | MouseEventFlags.Absolute), 0, 0, 0, IntPtr.Zero);
                mouse_event((int)(mEvent.UpEvent | MouseEventFlags.Absolute), 0, 0, 0, IntPtr.Zero);
            }
            finally
            {
        ...
        //线程主要处理的函数
        private void ThreadRunMethod(object sleepTime)
        {
            while (true)
            {
        ...
        private void BT_Start_Click(object sender, EventArgs e)
        {
            try
            {
                var sleepTime = int.Parse(DP_SleepTime.Text) * 1000;
                controlThread = new Thread(new ParameterizedThreadStart(ThreadRunMethod));
                controlThread.Start(sleepTime);
            }
            catch (Exception)
            {
